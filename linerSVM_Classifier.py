...
if __name__ == '__main__':
    ...
    for username in os.listdir( "./output/" ):
        ...
        for train_index, test_index in kf.split(X):
            ...
            """
            ###線形SVMによる分類（結果は疲労度3になるのが多く良くない）
            #Accuracy:0.297619, std:0.262334
            forest=svm.LinearSVC(loss='hinge', C=1.0,class_weight='balanced', random_state=0)#loss='squared_hinge' #loss="hinge", loss="log"
            forest.fit(X_train, y_train)
            """
            
            """
            ###k近傍による分類（結果は疲労度3になるのが多く良くない）
            #Accuracy:0.422222, std:0.171450
            forest=KNeighborsClassifier(n_neighbors=5) # インスタンス生成。n_neighbors:Kの数
            forest.fit(X_train, y_train)
            """
            
            ###SVMによる分類（結果は疲労度3になるのが多く、疲労度1,5の結果がなかった）
            #Accuracy:0.427381, std:0.200881
            forest=svm.SVC(kernel='rbf', gamma=1/2 , C=1.0,class_weight='balanced', random_state=0) #loss='squared_hinge' #loss="hinge", loss="log"
            forest.fit(X_train, y_train)
