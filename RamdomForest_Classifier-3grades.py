...
def print_cmx(y_true, y_pred):
    ...
    plt.savefig("confusion matrix-3grades.png", dpi=1200)
    plt.show()
...
if __name__ == '__main__':
    ...
    tmp=list()
    tmp=copy.deepcopy(df["lf/hf"])
    tmp[df["lf/hf"]<df["lf/hf"].quantile(.25)]="Low"
    tmp[df["lf/hf"]>df["lf/hf"].quantile(.75)]="High"
    tmp[(df["lf/hf"]<df["lf/hf"].quantile(.75))&(df["lf/hf"]>df["lf/hf"].quantile(.25))]="Middle"
    y=tmp
    ...
    df_result.to_csv("3Grade Classification Accuracy.csv")
    print("########## 結果 ###########")
